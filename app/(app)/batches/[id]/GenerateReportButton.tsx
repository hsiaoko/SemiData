...
export function GenerateReportButton({
  ...
  async function openOrFire() {
    setErr(null);
    // 普通用户：直接调 API，让后端用默认绑定规则
    if (!isAdmin) {
      setLoading(true);
      try {
  ...
  return (
    <div className="relative">
      ...
      {err && !open && <div className="text-2xs text-bin-c mt-1">{err}</div>}

      {open && isAdmin && (
        <div className="fixed inset-0 z-50 bg-ink/30 flex items-center justify-center p-6" onClick={() => !loading && setOpen(false)}>
          <div className="bg-surface border border-line shadow-card p-6 max-w-md w-full" onClick={(e) => e.stopPropagation()}>
            <div className="eyebrow mb-4">RULE SET · 选择评级规则</div>
            {ruleSets === null && !err && <div className="text-sm text-ink-3">加载规则集…</div>}
            {ruleSets && ruleSets.length === 0 && (
              <div className="text-sm text-bin-c">
                该数据集尚未绑定任何规则集，无法生成报告。请先在该数据集详情页绑定规则集后再试。
              </div>
            )}
            {ruleSets && ruleSets.length > 0 && (
              <div className="space-y-2 mb-5">
                {ruleSets.map((rs) => (
                  ...
                  >
                    <input type="radio" name="ruleSet" checked={selected === rs.id} onChange={() => setSelected(rs.id)} className="mt-1" />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-medium">{rs.name}</span>
                        {rs.isDefault && <span className="tag text-cobalt text-2xs">默认</span>}
                      </div>
                      {rs.description && <div className="serial mt-0.5 leading-relaxed">{rs.description}</div>}
                    </div>
                  </label>
                ))}
              </div>
            ...
            <div className="flex justify-end gap-3">
              <button type="button" className="btn-ghost" onClick={() => setOpen(false)} disabled={loading}>取消</button>
              <button type="button" className="btn-primary" onClick={go} disabled={loading || !selected || (ruleSets?.length ?? 0) === 0}>
